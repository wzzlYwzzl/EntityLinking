def hits_analyze(graph, iter):
    ...
    for _ in range(iter):
        for node in graph.nodes:
            ...
            node.authority_weight += tmp * node.authority_weight_for_calculation

            tmp = 0.0
            successors = graph.successors(node)
            for node_tmp in successors:
                tmp += node_tmp.authority_weight
            node.hub_weight += tmp * node.hub_weight_for_calculation
    ...
    for node in graph.nodes:
        node.authority_weight = node.authority_weight / sum_authority
        node.hub_weight = node.hub_weight / sum_hub